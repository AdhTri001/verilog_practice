...
$AssignmentFolder = $Assignment.ToLower()
...
# Check for PDF file
$PdfFile = "${Assignment}.pdf"
if (-not (Test-Path -Path $PdfFile)) {
    Write-Warning "PDF file '$PdfFile' not found. Continuing without it..."
}
...
if (Test-Path -Path $PdfFile) {
    $FilesToZip += (Get-Item $PdfFile).FullName
}
